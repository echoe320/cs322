...
namespace L1{
  ...
  void generate_code(Program p){
    ...
    //main function call
    string main_function = "_" + p.entryPointLabel.substr(1);
    outputFile << "\tcall " << main_function << "\n";
    ...
    for (auto func : p.functions) {
      string local_function = "_" + func->name.substr(1);
      outputFile << local_function + ":" << "\n";

      //extra args
      // if (func->arguments - 6 > 0){
        
      // }
      int64_t num_arg = func->arguments;
      int64_t num_locals = func->locals;
      int64_t temp_num = 0;
      temp_num += num_locals*8;
      //locals
      
      //for (int64_t i = num_locals; i > 0; i--){
      if (num_locals > 0) {
        outputFile << "\tsubq $" + to_string(temp_num) + ", %rsp\n";
      } 

      if (num_arg > 6) {
        num_arg = num_arg-6;
        temp_num = (num_locals + num_arg)*8;
      }
      ...
      for (auto i : func->instructions) {
        ...
        switch (i->id) {
          ...
          case assignment:
          {
            ...
            if ((inst->dst.isMem || inst->dst.isARegister)  && inst->src.isLabel){
              src = "$" + src;
            }
          ...
          case shift:
          {
            ...
            string src;
            if (inst->src.isARegister){
              src = "%cl";
            } else {
              src = ret_arg(inst->src, true);
            }
            instruction += op_arg + " " + src + ", " + dst;
            break;
          }
